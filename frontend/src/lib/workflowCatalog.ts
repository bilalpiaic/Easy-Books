...
export const DEMO_TENANTS: Record<DemoTenantKey, {
  ...
  pra:            { label: "FBR Digital Invoicing", email: "demo.pra@easy-books.app",            model: "pra_einvoice",        modules: ["base", "pra"] },
...
const TENANT_ENTRIES: CatalogEntry[] = [
  ...
  {
    id: "tenant-pra", kind: "tenant", title: "FBR Digital Invoicing retailer", href: "/pra-dashboard",
    captureTenant: "pra", segment: "PRA", modules: ["base", "pra"], tenants: ["pra"],
    tags: ["tenant", "compliance", "tax", "sales"],
    explanation: "Pakistani retail with FBR Digital Invoicing (PRAL DI API v1.12; historic add-on key pra). Every posted invoice can be validated then posted; submission logs keep the success/retry trail. Currency is PKR and the PRA home is the default operations landing.",
...
const SEGMENT_COPY: Record<string, { title: string; explanation: string; tags: string[]; href: string; captureTenant: DemoTenantKey }> = {
  ...
  pra:           { title: "FBR Digital Invoicing", href: "/pra-dashboard",     captureTenant: "pra",           tags: ["compliance", "tax"], explanation: "FBR DI sandbox dashboard and submission logs (PRAL v1.12). Posted invoices validate-then-post; failures retry without rewriting the GL." },
