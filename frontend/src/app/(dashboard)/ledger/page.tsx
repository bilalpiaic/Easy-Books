...
export default function LedgerPage() {
  ...
  useEffect(() => {
    apiFetch<any[]>("/api/reports/journal")
      .then(data => {
    ...
    <div className="p-8">
      ...
      <div className="space-y-8">
        {isLoading ? (
          ...
          <div className="text-center py-20 text-[#1a1814]/75">No transactions recorded yet.</div>
        ) : (
          ledgerData.map(([accountName, entries]: [any, any]) => (
            <div key={accountName} className="bg-white rounded-3xl shadow-xl shadow-black/5 border border-[#1a1814]/5 overflow-hidden">
              <div className="bg-[#f6f3ee] px-8 py-4 border-b border-[#1a1814]/5 flex justify-between items-center">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-[#1a1814]/75">Account Activity</span>
              </div>
